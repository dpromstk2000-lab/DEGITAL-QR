(function(){
  ...
  async function run(){O.clear(mainMessage);setBusy(true);try{const data=await O.api("/api/hybrid/owner/final-system-check");render(data);O.show(mainMessage,data.ok?"success":"error",data.ok?"営業前system-checkは全項目合格です。":"一部の項目を確認してください。") }catch(e){O.show(mainMessage,"error",e.message);$("rawOutput").textContent=JSON.stringify(e.data||{message:e.message},null,2)}finally{setBusy(false)}}
