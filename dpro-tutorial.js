...
function findTarget(){try{const doc=el.frame.contentDocument;const step=STEPS[state.index];for(const sel of [step.primary,...step.fallback]){const node=doc.querySelector(sel);if(node&&visibleInFrame(node)){resolved={node,selector:sel,fallback:sel!==step.primary};return resolved;}}for(const sel of [step.primary,...step.fallback]){const node=doc.querySelector(sel);if(node&&!dangerousSelector(sel)){try{node.scrollIntoView({block:'center',inline:'nearest',behavior:'auto'});}catch(_){}if(visibleInFrame(node)){resolved={node,selector:sel,fallback:sel!==step.primary};return resolved;}}} }catch(_){ } resolved=null;return null;}
